...
@pytest.mark.asyncio
async def test_suggest_layout_single_rack(
    rack_service,
    mock_rack_repository,
...
@pytest.mark.asyncio
async def test_suggest_layout_missing_racks(
    rack_service,
    mock_rack_repository,
    mock_device_repository,
    sample_racks,
...
@pytest.mark.asyncio
async def test_suggest_layout_missing_devices(
    rack_service,
    mock_rack_repository,
    mock_device_repository,
    sample_racks,
...
@pytest.mark.asyncio
async def test_suggest_layout_units_constraint(
...
@pytest.mark.asyncio
async def test_suggest_layout_power_constraint(
    rack_service, mock_rack_repository, mock_device_repository
):
    small_racks = [
...
async def test_suggest_layout_success(
    ...
    result = await rack_service.suggest_layout([1, 2, 3, 4], [1, 2, 3, 4, 5, 6, 7])

    assert isinstance(result, RackLayoutsResponse)
    assert len(result.layout) == 4

    placed_devices = []
    for layout in result.layout:
        placed_devices.extend(layout.devices)

    assert len(placed_devices) == 7
    assert len(placed_devices) == len(set(placed_devices))
