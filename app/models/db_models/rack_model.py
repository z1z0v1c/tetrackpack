...
class RackModel(SQLModel, table=True):
    ...
    max_power_consumption: int = Field(ge=5000)
    total_power_conssumption: int = Field(default=0)
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)

    devices: List["DeviceModel"] = Relationship(back_populates="rack")  # type: ignore
